...
// Browser environment
try { setGlobalObj(window) } catch (err) {
// Worker environment
try { setGlobalObj(self) } catch (err) {
// Node environment
try { setGlobalObj(global) } catch (err) {
// Unknow environment, check if globalThis exists
try { setGlobalObj(globalThis) } catch (err) {
// Unknow environment without globalThis, simulate a global object
